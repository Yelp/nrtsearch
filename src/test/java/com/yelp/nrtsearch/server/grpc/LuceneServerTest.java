...
public class LuceneServerTest {
  ...
  public void testReleaseSnapshotOnPrimary() throws IOException, InterruptedException {
    GrpcServer.TestServer testAddDocs =
            new GrpcServer.TestServer(grpcServer, true, Mode.PRIMARY);
    // 2 docs addDocuments
    testAddDocs.addDocuments();
    ...
    CreateSnapshotRequest createSnapshotRequest = CreateSnapshotRequest.newBuilder()
            .setIndexName(grpcServer.getTestIndex())
            .build();
    CreateSnapshotResponse createSnapshotResponse = grpcServer.getBlockingStub()
    ...
    createSnapshotResponse = grpcServer.getBlockingStub()
            .createSnapshot(createSnapshotRequest);
    ...
    ReleaseSnapshotRequest releaseSnapshotRequest = ReleaseSnapshotRequest.newBuilder()
            ...
            .build();
    ReleaseSnapshotResponse releaseSnapshotResponse = grpcServer.getBlockingStub()
    ...
    releaseSnapshotRequest = ReleaseSnapshotRequest.newBuilder()
            .setIndexName(grpcServer.getTestIndex())
            .setSnapshotId(SnapshotId.newBuilder().setIndexGen(2).setStateGen(0))
            .build();
    releaseSnapshotResponse = grpcServer.getBlockingStub()
            .releaseSnapshot(releaseSnapshotRequest);
    assertTrue(releaseSnapshotResponse.getSuccess());

    // Verify both index gens released
    GetAllSnapshotGenRequest getAllSnapshotGenRequest = GetAllSnapshotGenRequest.newBuilder()
            .setIndexName(grpcServer.getTestIndex())
            .build();
    GetAllSnapshotGenResponse getAllSnapshotGenResponse = grpcServer.getBlockingStub()
  ...
  public void testGetAllSnapshotIndexGen() throws IOException, InterruptedException {
    ...
    assertThat(getAllSnapshotGenResponse.getIndexGensList(), IsCollectionContaining.hasItems(1L, 2L));
  }
