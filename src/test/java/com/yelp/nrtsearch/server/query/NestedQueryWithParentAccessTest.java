  ...
  static class TestScriptFactory extends ScoreScript.SegmentFactory {
    ...
    public DoubleValues newInstance(LeafReaderContext ctx, DoubleValues scores) {
      if ("parent_access_test".equals(scriptId)) {
        return new ParentAccessDoubleScoreScript(factoryContext, ctx, scores);
      } else {
  ...
  public void testNestedQueryWithParentAccess() throws IOException {
    ...
    for (SearchResponse.Hit hit : response.getHitsList()) {
      ...
      if (score == -1.0) {
        ...
        throw new AssertionError("Failed to access parent field 'int_score' via _PARENT. prefix");
      } else if (score == -3.0) {
  ...
  static class ParentAccessDoubleScoreScript extends ScoreScript {
    ...
    public double execute() {
      ...
      try {
        ...
        // Use _PARENT. prefix to access parent field
        LoadedDocValues<?> parentField = segmentDocLookup.get("_PARENT.int_score");

        if (parentField == null) {
          return -2.0; // Failed to access parent field via _PARENT. prefix
        }
