 ...
 */
package com.yelp.nrtsearch.server.luceneserver.vector;
...
public class FloatVectorTypeTest {
  ...
  public void vectorTypeConstructorTest() {
    FloatVectorType vector = new FloatVectorType(TEST_ARRAY);
    assertEquals(TEST_ARRAY.length, vector.size());

    FloatVectorType vectorWithCapacity = new FloatVectorType(INITIAL_CAPACITY);
  ...
  public void vectorTypeExceptionTest() {
    Exception capacityException =
        Assert.assertThrows(IllegalArgumentException.class, () -> new FloatVectorType(-2));
    ...
    Exception illegalIndexException =
        Assert.assertThrows(
            ...
            () -> {
              FloatVectorType vector = new FloatVectorType(INITIAL_CAPACITY);
              vector.set(10, 1.0f);
            });
    assertTrue(
