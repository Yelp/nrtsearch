...
public class PrefixFieldDefTest {
  ...
  @Before
  public void init() {
    SimilarityCreator.initialize(getEmptyConfig(), Collections.emptyList());
  }

  private static LuceneServerConfiguration getEmptyConfig() {
    String config = "node: node1";
  ...
  public void testPrefixQueryLessThanMin() {
    ...
    assertTrue(((BooleanQuery) innerQuery).clauses().get(0).getQuery() instanceof AutomatonQuery);
    assertTrue(((BooleanQuery) innerQuery).clauses().get(1).getQuery() instanceof TermQuery);
  }
