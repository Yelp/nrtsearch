...
public class FloatFieldDefTest extends ServerTestCase {
  ...
  public FieldDefRequest getIndexDef(String name) throws IOException {
    return getFieldsFromResourceFile("/field/registerFieldsFloat.json");
  }
  ...
  private void rangeQuery(String fieldName) {
    ...
    assertRangeQuery(rangeQuery, 1f, 10f, 20f, 30f);
    ...
    rangeQuery =
        RangeQuery.newBuilder()
            ...
            .build();
    assertRangeQuery(rangeQuery, 10f, 20f, 30f);
    ...
    rangeQuery =
        RangeQuery.newBuilder()
            ...
            .build();
    assertRangeQuery(rangeQuery, 1f, 10f, 20f);
    ...
    rangeQuery =
        RangeQuery.newBuilder()
            ...
            .setUpperExclusive(true)
            .build();
    assertRangeQuery(rangeQuery, 10f, 20f);

    // Only upper bound defined

    // Both inclusive
    rangeQuery = RangeQuery.newBuilder().setField(fieldName).setUpper("20").build();
    assertRangeQuery(rangeQuery, Float.NEGATIVE_INFINITY, 1f, 10f, 20f);
    ...
    assertRangeQuery(rangeQuery, Float.NEGATIVE_INFINITY, 1f, 10f);
    ...
    rangeQuery =
        RangeQuery.newBuilder()
            ...
            .setUpperExclusive(true)
            .build();
    assertRangeQuery(rangeQuery, 20f, 30f);
  }

  private void assertRangeQuery(RangeQuery rangeQuery, Float... expectedValues) {
    ...
    List<Float> actualValues =
        searchResponse.getHitsList().stream()
            .map(
                hit -> hit.getFieldsMap().get(fieldName).getFieldValueList().get(0).getFloatValue())
            .sorted()
