...
public class VectorFieldDefEmbeddingTest {
  ...
  public void testParseVectorFieldWithTextEmbedding() {
    ...
    assertArrayEquals(floatsToBytes(MOCK_EMBEDDING), field.binaryValue().bytes);
  }
  ...
  public void testParseVectorFieldWithJsonArrayAndEmbeddingProvider() {
    ...
    IndexableField field = document.getFields().get(0);
    assertEquals("test_vector", field.name());
    assertArrayEquals(floatsToBytes(new float[] {1.0f, 2.0f, 3.0f}), field.binaryValue().bytes);
  }
  ...
  public void testParseVectorFieldWithTextNoProviderFails() {
    ...
    IllegalStateException ex =
        assertThrows(
            IllegalStateException.class, () -> fieldDef.parseVectorField("hello world", document));
    assertTrue(ex.getMessage().contains("No embedding provider configured for field"));
  }

  @Test
  public void testParseByteVectorFieldWithTextNoProviderFails() {
    ByteVectorFieldDef fieldDef = createByteFieldDefWithoutEmbedding();
    Document document = new Document();
  ...
  private ByteVectorFieldDef createByteFieldDefWithoutEmbedding() {
    Field requestField =
        Field.newBuilder()
            .setName("test_vector")
            .setType(FieldType.VECTOR)
  ...
  private static byte[] floatsToBytes(float[] floats) {
    ByteBuffer buf = ByteBuffer.allocate(Float.BYTES * floats.length);
