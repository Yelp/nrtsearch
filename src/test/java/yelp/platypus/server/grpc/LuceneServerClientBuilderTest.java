...
public class LuceneServerClientBuilderTest {
    ...
    public void buildRequest() throws IOException {
        ...
        Assert.assertEquals("first vendor", firstDoc.getFieldsMap().get("vendor_name").getValue(0));
        Assert.assertEquals("first again", firstDoc.getFieldsMap().get("vendor_name").getValue(1));
        Assert.assertEquals("3", firstDoc.getFieldsMap().get("count").getValue(0));
        AddDocumentRequest secondDoc = addDocumentRequestList.get(1);
        assertEquals("test_index", secondDoc.getIndexName());
        Assert.assertEquals("second vendor", secondDoc.getFieldsMap().get("vendor_name").getValue(0));
        Assert.assertEquals("second again", secondDoc.getFieldsMap().get("vendor_name").getValue(1));
        Assert.assertEquals("7", secondDoc.getFieldsMap().get("count").getValue(0));
