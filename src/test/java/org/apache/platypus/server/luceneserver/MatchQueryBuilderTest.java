...
import org.apache.platypus.server.grpc.FuzzyParams;
...
public class MatchQueryBuilderTest {
    ...
    private static final List<Term> MULTIPLE_TERMS = Stream.of("multiple", "terms", "here")
            .map(text -> new Term(FIELD, text))
            .collect(Collectors.toList());

    private static final FuzzyParams FUZZY_PARAMS = FuzzyParams.newBuilder()
            .setMaxEdits(2)
            .setPrefixLength(5)
            .setMaxExpansions(7)
    ...
    public void testSingleTerm() {
        MatchQueryBuilder matchQueryBuilder = new MatchQueryBuilder(getTestAnalyzer(), FuzzyParams.newBuilder().build());
    ...
    public void testSingleTermWithFuzzyParams() {
        ...
        FuzzyQuery expectedQuery = new FuzzyQuery(SINGLE_TERM, FUZZY_PARAMS.getMaxEdits(), FUZZY_PARAMS.getPrefixLength(), FUZZY_PARAMS.getMaxExpansions(), FUZZY_PARAMS.getTranspositions());
    ...
    public void testMultipleTermsWithFuzzyParams() {
        ...
        BooleanQuery.Builder builder = new BooleanQuery.Builder();
        MULTIPLE_TERMS.forEach(term -> {
            FuzzyQuery fuzzyQuery = new FuzzyQuery(term, FUZZY_PARAMS.getMaxEdits(), FUZZY_PARAMS.getPrefixLength(), FUZZY_PARAMS.getMaxExpansions(), FUZZY_PARAMS.getTranspositions());
