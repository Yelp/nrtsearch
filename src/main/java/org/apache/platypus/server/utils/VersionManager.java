    ...
    */
    ...
    long getListedLatestVersionNumber(String versionPath) throws IOException {
        ...
        if (s3.doesObjectExist(bucketName, latestVersionFile)) {
            try (
                    final S3Object s3Object = s3.getObject(bucketName, latestVersionFile);
            ) {
    ...
    */
    ...
    public boolean blessVersion(String serviceName, String resourceName, String resourceHash) {
        ...
        if (!s3.doesObjectExist(bucketName, resourceKey)) {
            logger.error(String.format("bless_version -- %s/%s/%s does not exist in s3", serviceName, resourceName, resourceHash));
            return false;
        }
        long newVersion;
        try {
        ...
        if (newVersion == 0) {
            logger.warn(String.format("This is the first blessing of this resource, %s, make sure it is not a typo", resourceName));
        }
