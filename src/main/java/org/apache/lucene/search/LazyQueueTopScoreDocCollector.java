...
/**
 * Modified version of Lucene TopScoreDocCollector, with the main difference that it passes false as
 * the prePopulate option to the HitQueue. This can provide a performance improvement in the case
 * where to numHits is greater than the totalHits.
 */
 ...
 */
public class LazyQueueTopScoreDocCollector extends TopDocsCollector<ScoreDoc> {
  static final ScoreDoc NO_VALUE = new ScoreDoc(Integer.MAX_VALUE, Float.NEGATIVE_INFINITY);
  ...
  public LeafCollector getLeafCollector(LeafReaderContext context) throws IOException {
    ...
    return new LeafCollector() {
      ...
      public void collect(int doc) throws IOException {
        ...
        float topScore;
        if (pq.size() == numHits) {
          topScore = pq.top().score;
        } else {
          topScore = Float.NEGATIVE_INFINITY;
        }
      ...
      private void collectCompetitiveHit(int doc, float score) throws IOException {
        if (pq.size() < numHits) {
          pq.add(new ScoreDoc(doc + docBase, score));
        } else {
          ScoreDoc pqTop = pq.top();
      ...
      private void updateMinCompetitiveScore(Scorable scorer) throws IOException {
        if (totalHits > totalHitsThreshold) {
          ...
          float localMinScore = Math.nextUp(top.score);
          if (localMinScore > minCompetitiveScore) {
            ...
            if (minScoreAcc != null) {
              ...
              minScoreAcc.accumulate(top.doc, top.score);
            }
          }
        }
      }
