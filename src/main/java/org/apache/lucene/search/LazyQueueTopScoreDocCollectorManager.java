...
/**
 * Modified version of Lucene TopScoreDocCollectorManager, which uses the
 * LayQueueTopScoreDocCollector.
 */
 ...
 */
public class LazyQueueTopScoreDocCollectorManager
    implements CollectorManager<LazyQueueTopScoreDocCollector, TopDocs> {
   ...
   * hits to collect and the number of hits to count accurately.
   *
   ...
   * @deprecated Use {@link #LazyQueueTopScoreDocCollectorManager(int, ScoreDoc, int)}, the
   *     supportsConcurrency parameter is now a no-op.
   */
  @Deprecated
  public LazyQueueTopScoreDocCollectorManager(
  ...
  /**
   * Creates a new {@link org.apache.lucene.search.TopScoreDocCollectorManager} given the number of
   * hits to collect and the number of hits to count accurately, with thread-safe internal states.
   *
   * <p><b>NOTE</b>: If the total hit count of the top docs is less than or exactly {@code
   * totalHitsThreshold} then this value is accurate. On the other hand, if the {@link
   * TopDocs#totalHits} value is greater than {@code totalHitsThreshold} then its value is a lower
   ...
   *     count accurate, but this will also make query processing slower.
   */
  public LazyQueueTopScoreDocCollectorManager(int numHits, ScoreDoc after, int totalHitsThreshold) {
   ...
   *     count accurate, but this will also make query processing slower.
   */
  public LazyQueueTopScoreDocCollectorManager(int numHits, int totalHitsThreshold) {
    this(numHits, null, totalHitsThreshold, true);
  }

  @Override
  public LazyQueueTopScoreDocCollector newCollector() {
    return new LazyQueueTopScoreDocCollector(numHits, after, totalHitsThreshold, minScoreAcc);
  }

  @Override
  public TopDocs reduce(Collection<LazyQueueTopScoreDocCollector> collectors) throws IOException {
