...
/**
 * Class that provides a lookup interface for doc values in a single lucene segment. Values are accessed through
 * the Map interface, with each field name mapping to its {@link LoadedDocValues}.
 *
 * The {@link LoadedDocValues} for each field are cached and can be reused for all subsequent documents in
 * the segment.
 */
public class SegmentDocLookup implements Map<String, LoadedDocValues<?>> {
    ...
    /**
     * Set target document id. All retrieved {@link LoadedDocValues} after this call will contain the doc values
     * for this id.
    ...
    /**
     * Check if a given field name is capable of having doc values. This does not mean there is data present,
     * just that there can be.
    ...
    /**
     * Get the {@link LoadedDocValues} for a given field. Creates a new instance or uses one from the cache.
     * The data is loaded for the current set document id.
     * @param key field name
     * @return {@link LoadedDocValues} implementation for the given field
     * @throws IllegalArgumentException if the field does not support doc values, if there is a problem
     * setting the target doc id, or if the field does not exist in the index
