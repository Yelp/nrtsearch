...
public class DeleteByQueryHandler implements Handler<DeleteByQueryRequest, AddDocumentResponse> {
    ...
    @Override
    public AddDocumentResponse handle(IndexState indexState, DeleteByQueryRequest deleteByQueryRequest) throws DeleteByQueryHandlerException {
        final ShardState shardState = indexState.getShard(0);
        indexState.verifyStarted();

        List<Query> queryList = deleteByQueryRequest.getQueryList()
                .stream()
