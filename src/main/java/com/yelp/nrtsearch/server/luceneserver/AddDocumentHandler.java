...
public class AddDocumentHandler implements Handler<AddDocumentRequest, Any> {
  ...
  public static class LuceneDocumentBuilder {
    ...
    private static void parseMultiValueField(
      ...
      if (fieldValues.size() > 1 && !field.multiValued) {
        throw new AddDocumentHandlerException(
            String.format("field: %s needs to be registered as multivalued", field.name));
      }
      Object nativeValue;
      ...
      if (field.valueType.equals(FieldDef.FieldValueType.LAT_LON)) {
        ...
        // TODO: in case of hierarchy fieldValue will be List<String> need to handle that separately
        // (ned to enhance proto:MultiValuedField for that)
    ...
    private static double[] getLatLonObject(FieldDef field, ProtocolStringList fieldValues)
        throws AddDocumentHandlerException {
      if (fieldValues.size() != 2) {
        throw new AddDocumentHandlerException(
            String.format(
                "field: %s is of type: %s and needs to be have latitude, longitude only",
      ...
      try {
        ...
        throw new AddDocumentHandlerException(
            "LAT_LON field type can only take in valid latitude/longitude format", e);
      }
    }

    /** Parses the current single value from MultiValuedField into the corresponding java object. */
    private static Object getNativeValue(FieldDef fd, String value)
        throws AddDocumentHandlerException {
      Object o;
      switch (fd.valueType) {
          // TODO: throw errors upon invalid user values e.g. non valid doubles, int, boolean etc
          // strings
        case TEXT:
        case ATOM:
          o = value;
        ...
        case DATE_TIME:
          ...
          String format =
              String.format(
                  ...
                  fd.name, s, fd.dateTimeFormat, fd.dateTimeFormat);
        ...
        default:
          throw new AddDocumentHandlerException(
              String.format("%s unknown value type for field %s", fd.valueType, fd.name));
      }
      return o;
    }
    ...
    private static void parseOneNativeValue(FieldDef fd, Document doc, Object o)
        throws AddDocumentHandlerException {
      assert o != null;
      assert fd != null;
      if (fd.faceted.equals(FieldDef.FacetValueType.FLAT)) {
        if (o instanceof List) {
          throw new AddDocumentHandlerException(
              String.format(
                  "%s value should be String when facet=flat; got %s", fd.name, o.getClass()));
        }
        doc.add(new FacetField(fd.name, o.toString()));
      } else if (fd.faceted.equals(FieldDef.FacetValueType.HIERARCHY)) {
        // TODO: hierarchy is broken right now (We need to support List<<List<String>> within
        // MultiValueFields in proto file
        ...
        if (o instanceof List) {
          throw new AddDocumentHandlerException(
              String.format(
                  "%s value should be String when facet=sortedSetDocValues; got %s",
                  fd.name, o.getClass()));
        }
      ...
      if (fd.highlighted) {
        ...
        if (fd.multiValued && (((String) o).indexOf(Constants.INFORMATION_SEP) != -1)) {
          ...
          // character:
          throw new AddDocumentHandlerException(
              "%s multiValued and hihglighted fields cannot contain INFORMATION_SEPARATOR (U+001F) character: this character is used internally when highlighting multi-valued fields"
                  .format(fd.name));
        }
      }
      ...
      if (dvType == DocValuesType.BINARY || dvType == DocValuesType.SORTED) {
        if (o instanceof String == false) {
          throw new AddDocumentHandlerException(
              String.format("%s expected String but got: %s", fd.name, o));
        }
        ...
        if (fd.valueType == FieldDef.FieldValueType.FLOAT) {
          if (fd.multiValued) {
            doc.add(
                new SortedNumericDocValuesField(
                    fd.name, NumericUtils.floatToSortableInt(((Number) o).floatValue())));
          } else {
          ...
          if (fd.multiValued) {
            doc.add(
                new SortedNumericDocValuesField(
                    fd.name, NumericUtils.doubleToSortableLong(((Number) o).doubleValue())));
          } else {
      ...
      if (fd.usePoints) {
        if (fd.valueType == FieldDef.FieldValueType.INT) {
          ...
          throw new AssertionError(
              String.format("fd.usePoints=true, invalid Type specified: %s", fd.valueType));
        }
      }
