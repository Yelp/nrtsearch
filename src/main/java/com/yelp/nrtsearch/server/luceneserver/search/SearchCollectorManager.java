 ...
 * implementation. Additional collection is provided by {@link CustomCollectorManager}
 * implementations.
 */
public class SearchCollectorManager
  ...
  private final List<CustomCollectorManager<? extends Collector, ? extends CollectorResult>>
      collectorManagers;
  ...
  public SearcherResult reduce(Collection<SearchCollector> collectors) throws IOException {
    ...
    if (collectorManagers.isEmpty()) {
      ...
      for (int i = 0; i < collectorManagers.size(); ++i) {
        ...
        CustomCollectorManager<? extends Collector, ? extends CollectorResult> cm =
            collectorManagers.get(i);

        // We must cast the CollectorManager to use the concrete Collector type. See above comment.
        @SuppressWarnings("unchecked")
        CollectorResult result =
            ((CustomCollectorManager<Collector, ? extends CollectorResult>) cm)
