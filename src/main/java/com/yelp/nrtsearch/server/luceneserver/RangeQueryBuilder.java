...
/**
 * Builds a range query for INT, LONG, FLOAT, DOUBLE and DATE_TIME types. Point based queries are
 * always created, and if docvalues are available for the field a docvalue query is also created and
 * both point and docvalue queries are wrapped in an {@link IndexOrDocValuesQuery} so that Lucene
 * runs whichever query is more efficient.
 */
  ...
  public Query buildRangeQuery(RangeQuery rangeQuery, IndexState state) {
    if (rangeQuery.getLower().isEmpty() && rangeQuery.getUpper().isEmpty()) {
      throw new IllegalArgumentException(
          "Neither lower nor upper bound provided in RangeQuery: " + rangeQuery);
    }
    ...
    switch (fieldValueType) {
      case INT:
        return getIntRangeQuery(rangeQuery, hasDocValues);
      case LONG:
        return getLongRangeQuery(rangeQuery, hasDocValues);
      case FLOAT:
        return getFloatRangeQuery(rangeQuery, hasDocValues);
      case DOUBLE:
        return getDoubleRangeQuery(rangeQuery, hasDocValues);
      case DATE_TIME:
        return getDateRangeQuery(rangeQuery, field, hasDocValues);
      default:
        throw new UnsupportedOperationException(
            "Range queries are not supported for field type: " + fieldValueType);
    }
  }
