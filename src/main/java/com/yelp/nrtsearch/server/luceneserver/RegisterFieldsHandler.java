    ...
    private FieldDef parseOneFieldType(IndexState indexState, Map<String, FieldDef> pendingFieldDefs, String fieldName, Field currentField) throws RegisterFieldsException {
        ...
        if (type == FieldDef.FieldValueType.TEXT || type == FieldDef.FieldValueType.ATOM) {
            ...
            if (ft.indexOptions() != IndexOptions.NONE) {
                ...
                if (currentField.getIndexOptions().equals(com.yelp.nrtsearch.server.grpc.IndexOptions.DOCS)) {
                    ft.setIndexOptions(IndexOptions.DOCS);
                } else if (currentField.getIndexOptions().equals(com.yelp.nrtsearch.server.grpc.IndexOptions.DOCS_FREQS)) {
                    ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS);
                } else if (currentField.getIndexOptions().equals(com.yelp.nrtsearch.server.grpc.IndexOptions.DOCS_FREQS_POSITIONS)) {
                    ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS);
                } else if (currentField.getIndexOptions().equals(com.yelp.nrtsearch.server.grpc.IndexOptions.DOCS_FREQS_POSITIONS_OFFSETS)) {
                    ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS);
                } else { //default option
                    ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS);
                }
