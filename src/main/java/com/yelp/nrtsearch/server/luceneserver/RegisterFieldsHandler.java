  ...
  private FieldDef parseOneFieldType(
    ...
    if (type == FieldDef.FieldValueType.TEXT || type == FieldDef.FieldValueType.ATOM) {
      ...
      if (ft.indexOptions() != IndexOptions.NONE) {
        ...
        if (!currentField.getTermVectors().equals(TermVectors.NO_TERMVECTORS)) {
          if (currentField.getTermVectors().equals(TermVectors.TERMS)) {
            ...
            ft.setStoreTermVectorOffsets(true);
              ...
              .getTermVectors()
              .equals(TermVectors.TERMS_POSITIONS_OFFSETS_PAYLOADS)) {
            ft.setStoreTermVectors(true);
            ft.setStoreTermVectorPositions(true);
            ft.setStoreTermVectorOffsets(true);
        ...
        if (currentField
            .getIndexOptions()
            .equals(com.yelp.nrtsearch.server.grpc.IndexOptions.DOCS)) {
          ft.setIndexOptions(IndexOptions.DOCS);
        } else if (currentField
            .getIndexOptions()
            .equals(com.yelp.nrtsearch.server.grpc.IndexOptions.DOCS_FREQS)) {
          ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS);
        } else if (currentField
            .getIndexOptions()
            .equals(com.yelp.nrtsearch.server.grpc.IndexOptions.DOCS_FREQS_POSITIONS)) {
          ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS);
        } else if (currentField
            .getIndexOptions()
            .equals(com.yelp.nrtsearch.server.grpc.IndexOptions.DOCS_FREQS_POSITIONS_OFFSETS)) {
          ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS);
        } else { // default option
          ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS);
        }
    ...
    boolean isIndexedTextField =
        type == FieldDef.FieldValueType.TEXT && ft.indexOptions() != IndexOptions.NONE;

    if (isAnalyzerDefined(currentField.getAnalyzer())) {
      // If the analyzer field is provided, use it to get an analyzer to use for both indexing and
      // search
      ...
      if (isAnalyzerDefined(currentField.getIndexAnalyzer())) {
        ...
        // If no index analyzer is provided for a text field that will be indexed (have doc values),
        // use the
        // StandardAnalyzer.
      ...
      if (isAnalyzerDefined(currentField.getSearchAnalyzer())) {
        ...
        searchAnalyzer =
            AnalyzerCreator.getInstance().getAnalyzer(currentField.getSearchAnalyzer());
      } else if (isIndexedTextField) {
        // If no search analyzer is provided for a text field that will be indexed (have doc
        // values), use the
        // StandardAnalyzer.
        searchAnalyzer = AnalyzerCreator.getStandardAnalyzer();
      } else {
        // No search analyzer was found or needed. Use the index analyzer which may be a valid
        // analyzer or
        // the dummyAnalyzer.
    ...
    if (facetType.equals(FacetType.HIERARCHY)) {
      ...
      if (type != FieldDef.FieldValueType.LONG
          && type != FieldDef.FieldValueType.INT
          && type != FieldDef.FieldValueType.FLOAT
          && type != FieldDef.FieldValueType.DOUBLE) {
      ...
      // We index the field as points, for drill-down, and store doc values, for dynamic facet
      // counting
      ft.setDocValuesType(DocValuesType.NUMERIC);
      ...
      if (ft.indexOptions() == IndexOptions.NONE
          && ft.stored() == false
          && ft.docValuesType() == DocValuesType.NONE
          && usePoints == false) {
        throw new RegisterFieldsException(
            String.format(
                "field %s does nothing: it's neither searched, stored, sorted, grouped, highlighted nor faceted",
                fieldName));
      }
    }
    ...
    if (facetType.equals(FacetType.NO_FACETS) == false
        && facetType.equals(FacetType.NUMERIC_RANGE) == false) {
