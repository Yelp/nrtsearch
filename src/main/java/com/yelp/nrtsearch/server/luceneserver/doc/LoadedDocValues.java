...
/**
 * Container class for loading and holding doc values for a field. Abstracts loaded values as a list.
 * The various implementations of this class use the lucene segment doc values accessors to load the data
 * for that field.
 *
 * These are used during field data retrieval when building a search response, and provided to scripts during
 * execution.
 *
 * All implementations must define setDocId to advance the doc values accessor to the provided segment document.
 * All implementations must also define toFieldValue, which provides a
 * {@link com.yelp.nrtsearch.server.grpc.SearchResponse.Hit.FieldValue} containing the doc value data for a given
 * index in the list.
 *
 * All implementations throw an IllegalStateException when trying to get a value when there are no values for the field.
 * All implementations throw an IndexOutOfBoundsException when trying to access an invalid index.
 *
 * @param <T> the loaded doc values type. This could be a simple boxed primitive, or something more complex like
 *           a {@link GeoPoint}.
 */
public abstract class LoadedDocValues<T> extends AbstractList<T> {
    ...
    private static final LongFunction<Float> FLOAT_DECODER = (longValue) -> Float.intBitsToFloat((int) longValue);
    private static final LongFunction<Float> SORTED_FLOAT_DECODER = (longValue) -> NumericUtils.sortableIntToFloat((int) longValue);
    private static final LongFunction<Double> DOUBLE_DECODER = Double::longBitsToDouble;
    private static final LongFunction<Double> SORTED_DOUBLE_DECODER = NumericUtils::sortableLongToDouble;
    private static final LongFunction<Instant> DATE_DECODER = Instant::ofEpochMilli;
    private static final LongFunction<GeoPoint> GEO_POINT_DECODER = (longValue) -> new GeoPoint(
    ...
    public static abstract class SingleNumericValue<T> extends LoadedDocValues<T> {
    ...
    public static abstract class SortedNumericValues<T> extends LoadedDocValues<T> {
    ...
    public static final class Locations extends SortedNumericValues<GeoPoint> {
        ...
        public SearchResponse.Hit.FieldValue toFieldValue(int index) {
            ...
            LatLng latLon = LatLng.newBuilder().setLatitude(point.getLatitude()).setLongitude(point.getLongitude()).build();
    ...
    public static abstract class SingleBinaryBase<T> extends LoadedDocValues<T> {
    ...
    public static final class SingleBinary extends SingleBinaryBase<BytesRef> {
        ...
        @Override
        public SearchResponse.Hit.FieldValue toFieldValue(int index) {
            return SearchResponse.Hit.FieldValue.newBuilder().setTextValue(get(index).utf8ToString()).build();
        }
    }
