...
/**
 * A simple implementation of <a href="https://en.wikipedia.org/wiki/Reservoir_sampling">reservoir sampling</a> used
 * for having a set of warming queries.
 */
public class ReservoirSampler {
     ...
     * @return a {@link SampleResult}. If the query must be included, {@link SampleResult#isSample()} is {@code true}
     * and {@link SampleResult#getReplace()} points to the index of query in the list to replace current query with.
     * If the query must not be included then {@code isSample} returns {@code false}.
     */
    public SampleResult sample(String indexName) {
        long numQueriesForIndex = numQueries.computeIfAbsent(indexName, s -> new AtomicLong(0)).getAndIncrement();
