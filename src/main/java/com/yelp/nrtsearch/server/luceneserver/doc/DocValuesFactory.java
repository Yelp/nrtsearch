...
public class DocValuesFactory {
    ...
    /**
     * Get the correct {@link LoadedDocValues} implementation for the given {@link FieldDef}, and bound to
     * the provided lucene segment context.
     ...
     */
    public static LoadedDocValues<?> getDocValues(FieldDef field, LeafReaderContext context) throws IOException {
        ...
        switch(field.fieldType.docValuesType()) {
            ...
            case SORTED_SET:
                SortedSetDocValues sortedSetDocValues = DocValues.getSortedSet(context.reader(), field.name);
                return new LoadedDocValues.SortedStrings(sortedSetDocValues);
            default:
                throw new IllegalArgumentException("Unable to load doc values for field: " +
                        field.name + ", doc value type: " + field.fieldType.docValuesType());
        }
    }

    private static LoadedDocValues<?> getNumericForValueType(FieldDef field, LeafReaderContext context, FieldDef.FieldValueType valueType) throws IOException {
        ...
        switch (valueType) {
            ...
            default:
                throw new IllegalArgumentException("Unable to create doc value loader for field: " + field.name + ", type: " + valueType);
        }
    }

    private static LoadedDocValues<?> getSortedNumericForValueType(FieldDef field, LeafReaderContext context, FieldDef.FieldValueType valueType) throws IOException {
        SortedNumericDocValues sortedNumericDocValues = DocValues.getSortedNumeric(context.reader(), field.name);
        switch (valueType) {
            case BOOLEAN:
