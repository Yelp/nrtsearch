    ...
    public static void SetAnalyzer(String key, Map<String, Analyzer> map) throws Exception
    {
        switch (key) {
            ...
            case IndexedFieldsNames.TYPE: // TODO how to set normalizer + split on white space in order to lookup on this?
                Arrays.asList(IndexedFieldsExtensionsNames.LITE_DELIMITION, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION , IndexedFieldsExtensionsNames.PREFIXES )
                    .forEach(ext -> map.put(String.format("%s.%s", key, ext), GetIndexAnalyzer(key, ext)));
                
                // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.LITE_DELIMITION), new TypeLiteDelimitionAnalyzer());    
                // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION), new TypeAggressiveDelimitionAnalyzer());    
                // // ---> todo // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.PREFIXES), new TypePrefixAnalyzer()); 
                break;
            ...
            case IndexedFieldsNames.CONTENT:
                ...
                //map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.STEMMING), new TextStemmingAnalyzer()); 
                break;

            case IndexedFieldsNames.NAME:
                map.put(key, GetIndexAnalyzer(key));
                
                Arrays.asList(IndexedFieldsExtensionsNames.LITE_DELIMITION,
                ...
                // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.LITE_DELIMITION), new TextLiteDelimitionAnalyzer()); 
                // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION), new TextAggressiveDelimitionAnalyzer()); 
                // // ---> todo //map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION_PREFIX), new TextAggressiveDelimitionPrefixAnalyzer());  // TODO how to configure search analyzer?
                // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.SHINGLES), new TextShingleAnalyzer()); 
                // // ---> todo //map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.PREFIXES), new TextPrefixAnalyzer()); // TODO search analyzer?
                // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.STEMMING), new TextStemmingAnalyzer()); 
                // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.KEYWORD), new KeywordAnalyzer()); 
                break;
            case IndexedFieldsNames.TEXT:
                map.put(key, GetIndexAnalyzer(key));
            ...
            case IndexedFieldsNames.HIGHLIGHT:
                map.put(key, GetIndexAnalyzer(key)); // TODO removed as it is a prefix analyzer and we can search without it
            ...
            //     map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.LITE_DELIMITION), new PathLiteDelimitionAnalyzer());
            //     // ---> todo //map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.LITE_DELIMITION_PREFIX), new PathLiteDelimitionPrefixAnalyzer()); // TODO search analyzer
            //     map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION), new TextAggressiveDelimitionAnalyzer());
            //    // ---> todo // map.put(String.format("%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION_PREFIX), new TextAggressiveDelimitionPrefixAnalyzer()); // TODO search analyzer
                break;
