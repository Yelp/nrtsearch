...
public class FieldTypeProvider {
  ...
  private static FieldType CustomizedText(
      IndexOptions options, Boolean withTermVectorPositions, Boolean withTermVectorOffsets) {
  ...
  public static Document AddField(String key, String value, Document doc)
      throws Exception { // TODO all text fields here have a default termvector and indexoptions
    // different from ES. We should change this...
    ...
    switch (key) {
      ...
      case IndexedFieldsNames
          .APP_ID: // TODO how to set normalizer + split on white space in order to lookup on this?
      ...
      case IndexedFieldsNames
        ...
        doc.add(
            new StringField(key, value, Field.Store.YES)); // TODO change indexoptions for subfields
        doc.add(
            new TextField(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.LITE_DELIMITION),
                value,
                Field.Store.YES));
        doc.add(
            new TextField(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION),
                value,
                Field.Store.YES));
        doc.add(
            new TextField(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.PREFIXES),
                value,
                Field.Store.YES));
        // doc.add(new TextField(String.format("%s.%s", key, IndexedFieldsExtensionsNames.LENGTH),
        // value, Field.Store.YES)); // TODO removed
        break;
      ...
      case IndexedFieldsNames.CONTENT:
        ...
        doc.add(
            TextWithOffsets(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.STEMMING), value));
        break;
      case IndexedFieldsNames.NAME:
        doc.add(TextWithOffsets(key, value));
        doc.add(
            TextWithOffsets(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.LITE_DELIMITION), value));
        doc.add(
            TextWithOffsets(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION),
                value));
        doc.add(
            TextWithOffsets(
                String.format(
                    "%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION_PREFIX),
                value));
        doc.add(
            TextWithOffsets(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.SHINGLES), value));
        doc.add(
            TextWithOffsets(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.PREFIXES), value));
        doc.add(
            TextWithOffsets(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.STEMMING), value));
        ...
        doc.add(
            new TextField(
                String.format("%s.%s", key, IndexedFieldsExtensionsNames.KEYWORD),
                value,
                Field.Store.YES));
        break;
  ...
  private static final String[] _timestampTraits =
      new String[] {"sharedAt", "modifiedAt", "createdAt", "start"};

  private static void AddAnalyzedTimestamp(LinkedHashMap<String, Object> traits, Document doc) {
    Optional<Object> timestampTrait =
        Arrays.stream(_timestampTraits).map(x -> traits.get(x)).filter(o -> o != null).findFirst();
    ...
    long epoch =
        ISODateTimeFormat.dateTimeParser()
            .parseDateTime(timestampTrait.get().toString())
            .toInstant()
            .getMillis();
    ...
    switch (key) {
      case IndexedFieldsNames.PATHS:
        for (String v : value) {
          ...
          doc.add(
              TextWithOffsets(
                  String.format("%s.%s", key, IndexedFieldsExtensionsNames.LITE_DELIMITION), v));
          doc.add(
              TextWithOffsets(
                  String.format("%s.%s", key, IndexedFieldsExtensionsNames.LITE_DELIMITION_PREFIX),
                  v));
          doc.add(
              TextWithOffsets(
                  String.format("%s.%s", key, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION),
  ...
  public static Document AddField(String key, Date value, Document doc) throws Exception {
    switch (key) {
      case IndexedFieldsNames.INDEX_TIME:
        doc.add(
            new StringField(key, DateTools.dateToString(value, Resolution.HOUR), Field.Store.YES));
        break;
      default:
        throw new Exception(String.format("Unsupported field key: %s", key));
    }
