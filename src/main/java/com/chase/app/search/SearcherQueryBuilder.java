...
public class SearcherQueryBuilder {
  ...
  public Query ApplyPrefilter(Query query, SearchFilter filter) {
    ...
    if (filter.resourceType != null) {
      _builder.add(
          TermsInSet(Field(IndexedFieldsNames.TYPE).Field, filter.resourceType), Occur.FILTER);
    }
  ...
  public Query BuildInnerQuery(String query) { // TODO move to a separate class
    ...
    _builder.add(
        BuildPositiveQuery(query),
        Occur.MUST); // TODO there should be a "boosting" query here, think how to simulate with
    // Lucene. We'll only use the positive for now.
    return _builder.build();
  }
  ...
  public Query BuildPositiveQuery(String query) {
    // TODO : in ES we had a content-based dismax query here. However we don't enable content search
    // at this point so currently ignored.
  ...
  public Query BuildResourcesQuery(String query) {
    ...
    _builder.add(
        MultiMatch(
            ...
            Field(
                IndexedFieldsNames.NAME, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION, 0.3F),
    ...
    _builder.add(
        MultiMatch(
            ...
            Field(
                IndexedFieldsNames.TEXT, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION, 0.3F),
    ...
    _builder.add(
        MatchPhrase(
            query,
            Field(IndexedFieldsNames.NAME, IndexedFieldsExtensionsNames.PREFIXES).Field,
            6F,
            1),
        Occur.SHOULD);
    _builder.add(
        MatchPhrase(
            query,
            Field(IndexedFieldsNames.TEXT, IndexedFieldsExtensionsNames.PREFIXES).Field,
            4F,
            1),
        Occur.SHOULD);
    _builder.add(
        Match(
            query,
            Field(IndexedFieldsNames.PATHS, IndexedFieldsExtensionsNames.LITE_DELIMITION_PREFIX)
                .Field,
            MatchOperator.And,
            0.6F),
        Occur.SHOULD);
    ...
    _builder.add(
        MultiMatch(
            ...
            Field(
                IndexedFieldsNames.TYPE, IndexedFieldsExtensionsNames.AGGRESSIVE_DELIMITION, 0.6F),
            Field(IndexedFieldsNames.TYPE, IndexedFieldsExtensionsNames.PREFIXES, 0.1F)),
        Occur.SHOULD);

    // TODO support fuzziness - idea would be to change the "term" implementation to support
    // fuzziness - hence replacing "TermQuery" with "FuzzyQuery". The rest will simply bubble the
    // fuzzy request down.
  ...
  public Query AllowBoost(Query q, Float boost) {
    if (boost == null || boost == 1) return q;
  ...
  public Query Term(String query, FieldMetadata meta, Float boost) {
    Query q =
        new TermQuery(
            new Term(
                meta.FullName(),
                query)); /// TODO - does lucene analyze the text? otherwise why use the field name?
    return AllowBoost(q, boost);
  }

  public Query Synonym(FieldMetadata meta, Float boost, String... queries) {
    org.apache.lucene.search.SynonymQuery.Builder builder =
        new SynonymQuery.Builder(meta.FullName());
  ...
  public Query MatchPhrase(String query, FieldMetadata meta, Float boost, Integer slop) {
    ...
    try {
      ...
      tokens.stream()
          .sorted(Comparator.comparingInt(TokenData::GetPosition))
  ...
  public Query TermsInSet(FieldMetadata field, String[] terms) {
    return new TermInSetQuery(
        field.FullName(), Arrays.stream(terms).map(s -> new BytesRef(s)).toArray(BytesRef[]::new));
  }

  public Query Match(String query, FieldMetadata meta, MatchOperator operator, Float boost)
        //  throws IOException
      {
    // TODO: elasticsearch return the sum of all matches for all tokens. Does Lucene do the same the
    // way we used in bool query?
    if (!meta.Analyzed) {
      return Term(query, meta, boost);
    }
      ...
      tokens.stream()
          ...
          .map(
              s -> {
                ...
                return Synonym(
                    meta, boost, s.getValue().stream().map(x -> x.Text).toArray(String[]::new));
              })
  ...
  private Query BestFieldMultiMatch(
      String query, Float tieBreaker, MatchOperator op, FieldRequest... fields) {
    List<Query> disjuncts =
        Arrays.asList(
            Arrays.stream(fields)
                .map(f -> this.Match(query, f.Field, op, f.Boost))
                .toArray(Query[]::new));
    return new DisjunctionMaxQuery(disjuncts, tieBreaker == null ? 0 : tieBreaker);
  }

  private Query MostFieldsMultiMatch(
      String query,
      Float tieBreaker,
      MatchOperator op,
    ...
    Arrays.stream(fields)
        .map(f -> this.Match(query, f.Field, op, f.Boost))
        .forEach(q -> _builder.add(q, Occur.SHOULD));
    return _builder.build();
  }

  public Query MultiMatch(
      String query,
      MultiMatchType type,
      Float tieBreaker,
      MatchOperator op,
      Float boost,
      Integer minimumShouldMatch,
      FieldRequest... fields) // TODO  boost,fuzzy,
      {
    switch (type) {
      case BestField:
      ...
      case MostFields:
        return AllowBoost(
            MostFieldsMultiMatch(query, tieBreaker, op, minimumShouldMatch, fields), boost);
      default:
  ...
  public Query MultiMatch(String query, Float tieBreaker, Float boost, FieldRequest... fields) {
    return MultiMatch(
        query, MultiMatchType.BestField, tieBreaker, MatchOperator.Or, boost, null, fields);
  }
}
