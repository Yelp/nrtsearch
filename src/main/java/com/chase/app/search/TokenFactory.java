...
public class TokenFactory { // TODO rename asap
  ...
  public TokenFactory() {
    ...
    loader =
        new CacheLoader<String, ArrayList<TokenData>>() {
          @Override
  ...
  public ArrayList<TokenData> AnalyzeIntoTokens(FieldMetadata field, String text)
      throws IOException {
    String fieldName =
        field.Extension == null ? field.Name : String.format("%s.%s", field.Name, field.Extension);
    ...
    if (res == null) {
      res =
          AnalysisHelper.Analyze(
              fieldName,
              text,
              field.SearchAnalyzer == null ? field.Analyzer : field.SearchAnalyzer);
