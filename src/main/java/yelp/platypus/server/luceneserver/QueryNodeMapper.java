...
import yelp.platypus.server.grpc.MatchOperator;
import yelp.platypus.server.grpc.MatchPhraseQuery;
import yelp.platypus.server.grpc.MatchQuery;
import yelp.platypus.server.grpc.MultiMatchQuery;
import yelp.platypus.server.grpc.QueryType;
import yelp.platypus.server.grpc.RangeQuery;
...
import static yelp.platypus.server.luceneserver.AnalyzerCreator.getAnalyzer;
import static yelp.platypus.server.luceneserver.AnalyzerCreator.isAnalyzerDefined;
...
class QueryNodeMapper {
    ...
    private final Map<yelp.platypus.server.grpc.BooleanClause.Occur, BooleanClause.Occur> occurMapping
    ...
    Query getQuery(yelp.platypus.server.grpc.Query query, IndexState state) {
    ...
    private Query getQueryNode(yelp.platypus.server.grpc.Query query, IndexState state, QueryType queryType) {
        switch (queryType) {
    ...
    private BooleanQuery getBooleanQuery(yelp.platypus.server.grpc.BooleanQuery booleanQuery, IndexState state) {
    ...
    private PhraseQuery getPhraseQuery(yelp.platypus.server.grpc.PhraseQuery phraseQuery) {
    ...
    private FunctionScoreQuery getFunctionScoreQuery(
        yelp.platypus.server.grpc.FunctionScoreQuery functionScoreQuery,
        IndexState state) {
        Expression expr;
    ...
    private Query getTermQuery(yelp.platypus.server.grpc.TermQuery termQuery, IndexState state) {
    ...
    private Query getTermInSetQuery(yelp.platypus.server.grpc.TermInSetQuery termInSetQuery, IndexState state) {
    ...
    private DisjunctionMaxQuery getDisjunctionMaxQuery(
        yelp.platypus.server.grpc.DisjunctionMaxQuery disjunctionMaxQuery,
        IndexState state) {
    ...
    private Map<yelp.platypus.server.grpc.BooleanClause.Occur, BooleanClause.Occur> initializeOccurMapping() {
        return Arrays.stream(yelp.platypus.server.grpc.BooleanClause.Occur.values())
                .filter(v -> v != yelp.platypus.server.grpc.BooleanClause.Occur.UNRECOGNIZED)
                .collect(
                        () -> new EnumMap<>(yelp.platypus.server.grpc.BooleanClause.Occur.class),
