...
import yelp.platypus.server.grpc.FacetType;
import yelp.platypus.server.grpc.Field;
import yelp.platypus.server.grpc.FieldDefRequest;
import yelp.platypus.server.grpc.FieldDefResponse;
import yelp.platypus.server.grpc.FieldType;
import yelp.platypus.server.grpc.TermVectors;
...
//                }
    ...
    private FieldDef parseOneFieldType(IndexState indexState, Map<String, FieldDef> pendingFieldDefs, String fieldName, Field currentField) throws RegisterFieldsException {
        ...
        switch (type) {
            ...
            case ATOM:
                if (AnalyzerCreator.hasAnalyzer(currentField)) {
        ...
        if (AnalyzerCreator.hasAnalyzer(currentField) && ft.indexOptions() == IndexOptions.NONE) {
        ...
        if (type == FieldDef.FieldValueType.TEXT || type == FieldDef.FieldValueType.ATOM) {
            ...
            if (ft.indexOptions() != IndexOptions.NONE) {
                ...
                if (currentField.getIndexOptions().equals(
                    yelp.platypus.server.grpc.IndexOptions.DOCS)) {
                    ft.setIndexOptions(IndexOptions.DOCS);
                } else if (currentField.getIndexOptions().equals(
                    yelp.platypus.server.grpc.IndexOptions.DOCS_FREQS)) {
                    ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS);
                } else if (currentField.getIndexOptions().equals(
                    yelp.platypus.server.grpc.IndexOptions.DOCS_FREQS_POSITIONS)) {
                    ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS);
                } else if (currentField.getIndexOptions().equals(
                    yelp.platypus.server.grpc.IndexOptions.DOCS_FREQS_POSITIONS_OFFSETS)) {
                    ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS);
                } else { //default option
                    ft.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS);
                }
        ...
        if (AnalyzerCreator.isAnalyzerDefined(currentField.getAnalyzer())) {
            ...
            if (AnalyzerCreator.isAnalyzerDefined(currentField.getIndexAnalyzer())) {
            ...
            if (AnalyzerCreator.isAnalyzerDefined(currentField.getSearchAnalyzer())) {
