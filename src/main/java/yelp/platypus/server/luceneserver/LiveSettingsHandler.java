...
import yelp.platypus.server.grpc.LiveSettingsResponse;
...
public class LiveSettingsHandler implements Handler<LiveSettingsRequest, LiveSettingsResponse> {
    ...
    @Override
    public LiveSettingsResponse handle(IndexState indexState, LiveSettingsRequest liveSettingsRequest) {
        logger.info(String.format("update liveSettings for index:  %s", liveSettingsRequest.getIndexName()));
        ...
        if (liveSettingsRequest.getMaxSearcherAgeSec() != 0) {
            ...
            logger.info(String.format("set maxSearcherAgeSec: %s", liveSettingsRequest.getMaxSearcherAgeSec()));
        }
        if (liveSettingsRequest.getIndexRamBufferSizeMB() != 0) {
            ...
            logger.info(String.format("set indexRamBufferSizeMB: %s", liveSettingsRequest.getIndexRamBufferSizeMB()));
        }
