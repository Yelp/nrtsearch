...
RUN mkdir -p /root/lucene/server/plugins/chaseplugin/ && cp /code/build/libs/* /root/lucene/server/plugins/chaseplugin/
...
ENTRYPOINT ["/bin/bash", "-c", "/code/bin/http_wrapper-linux-amd64 6000 6002 & /code/build/install/nrtsearch/bin/lucene-server /root/lucene/server/config.yaml"]